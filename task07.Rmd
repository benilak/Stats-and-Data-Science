...
# ![some caption](.scatterplot-1.png)
# way to display an image file (plot) in r-markdown if the plot is already saved
# greatly reduces knitting time
# (it's markdown language, it goes inline (without `back ticks`), not in an r chunk)
# however, the easier thing to do is just set `cache = TRUE` in the knitr chunk options
```
